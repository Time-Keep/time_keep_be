class DemographicFacade
  def self.retailer_count(lat, lon)
    TomTomService.jewelry_retailers(lat,lon)[:summary][:totalResults]
  end

  def self.retailer_count2(lat, lon)
    TomTomService.jewelry_retailers2(lat,lon)[:summary][:totalResults]
  end
